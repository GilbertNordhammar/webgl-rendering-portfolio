...
void main()
{
    ...
    vec3 lightDir = directionalLights[0].direction; // Directional light
    vec3 halfway = normalize(lightDir + viewDir);
    vec3 radianceLight = directionalLights[0].color;
    ...
    float normalDotLightDir = max(0.0, dot(N, lightDir));

    vec3 radianceOut = (diffuse * albedo / PI + specular) * radianceLight * normalDotLightDir;
